    ...
    companion object {
        ...
        val MIGRATION_5_6 =
            object : Migration(5, 6) {
                override fun migrate(db: SupportSQLiteDatabase) {
                    db.execSQL("ALTER TABLE `presets` RENAME TO `presets_legacy`")
                    db.execSQL(
                    ...
                    db.execSQL(
                        "INSERT INTO `presets` (`id`, `name`, `settings_json`, `created_at`, `updated_at`) " +
                            "SELECT `id`, `name`, `settings_json`, `created_at`, `updated_at` FROM `presets_legacy`",
                    )
                    db.execSQL("DROP TABLE `presets_legacy`")
